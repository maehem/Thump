...
import static thump.game.play.Local.FLOATSPEED;
import static thump.game.play.Local.MELEERANGE;
import thump.game.play.Random;
import thump.wad.map.Line;
import static thump.wad.map.Line.ML_SOUNDBLOCK;
import static thump.wad.map.Line.ML_TWOSIDED;
import thump.game.maplevel.MapSector;
import thump.game.maplevel.MapSideDef;
import static thump.wad.map.Degenmobj.MobileObjectFlag.MF_CORPSE;
...
public class Enemy {
    ...
    private void P_RecursiveSound( MapSector sec, int soundblocks ) {
        ...
        for (i=0 ;i<sec.linecount ; i++) {
            ...
            //MapSideDef sides[] = Game.getInstance().playerSetup.sides;
            ArrayList<MapSideDef> sides = Game.getInstance().playerSetup.sides;
...
//            }
    ...
    public void P_NoiseAlert(MapObject target, MapObject emmiter) {
        ...
        P_RecursiveSound(emmiter.subsector.mapSector, 0);
    }
    ...
    boolean P_Move (MapObject	actor) {
        ...
        if (actor.movedir.ordinal() >= 8) {
            ...
            thump.base.Defines.logger.severe("Weird actor.movedir!");
        }
    ...
    void P_NewChaseDir(MapObject actor) {
        ...
        if (null == actor.target) {
            ...
            thump.base.Defines.logger.severe("P_NewChaseDir(): called with no target");
        }
