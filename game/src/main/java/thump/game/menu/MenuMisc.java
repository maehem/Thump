...
public class MenuMisc {
    ...
    int sn = 0;
    PatchData [] font = Game.getInstance().headUp.hu_font;
	...
	if (direct) {
            game.renderer.video.drawPatchDirect(xPos, y, 0, font[c], game.wad);
        } else {
            game.renderer.video.drawPatch(xPos, y, 0, font[c]);
        }
	xPos+=w;
    }
