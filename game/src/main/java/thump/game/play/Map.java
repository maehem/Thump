...
import static thump.game.State.StateNum.S_GIBS;
...
import static thump.game.play.Local.USERANGE;
import static thump.game.play.MapUtil.PT_ADDLINES;
import static thump.game.play.MapUtil.PT_ADDTHINGS;
import thump.game.play.PITfuncs.PIT_ChangeSector;
import thump.game.play.PITfuncs.PIT_CheckLine;
import thump.game.play.PITfuncs.PIT_CheckThing;
import thump.game.play.PITfuncs.PIT_RadiusAttack;
import thump.game.play.PITfuncs.PIT_StompThing;
import thump.game.play.PTRfuncs.PTR_AimTraverse;
import thump.game.play.PTRfuncs.PTR_ShootTraverse;
import thump.game.play.PTRfuncs.PTR_SlideTraverse;
import thump.game.play.PTRfuncs.PTR_UseTraverse;
import thump.base.BoundingBox;
import thump.game.maplevel.MapNode;
import thump.game.maplevel.MapObject;
import static thump.game.sound.sfx.Sounds.SfxEnum.sfx_noway;
import static thump.wad.map.Degenmobj.MobileObjectFlag.*;
import thump.wad.map.Line;
import static thump.wad.map.Line.*;
import static thump.wad.map.Line.Slopetype.ST_HORIZONTAL;
import static thump.wad.map.Line.Slopetype.ST_VERTICAL;
...
public class Map {
    ...
    public boolean PTR_SlideTraverse (Intercept in) {
        ...
        if (!in.isaline) {
            ...
            thump.base.Defines.logger.severe("PTR_SlideTraverse: not a line?");
        }
   ...
   private void isblocking(Intercept in, Line li) {
    ...
    public boolean PTR_ShootTraverse (Intercept in) {
        ...
        if (in.isaline) {
            ...
            if (li.frontsector.getCeilingPic(game.wad) == game.renderer.skyflatnum)
            {
                // don't shoot the sky!
                ...
                if	(li.backsector!=null && li.backsector.getCeilingPic(game.wad) == game.renderer.skyflatnum) {
                    return false;
                }		
            }
