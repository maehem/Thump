...
import thump.game.play.PTRfuncs.PTRfunc;
import thump.game.play.PITfuncs.PITfunc;
...
import static thump.game.play.Intercept.MAXINTERCEPTS;
import static thump.game.play.Local.MAPBLOCKSHIFT;
import static thump.game.play.Local.MAPBLOCKSIZE;
...
//    #endif
    ...
    public void P_SetThingPosition(MapObject thing) {
        ...
        // link into subsector
        ss = MapNode.R_PointInSubsector(thing.x, thing.y);
        thing.subsector = ss;
...
//        }
    ...
    public boolean P_BlockThingsIterator(int x, int y, PITfunc func) {//boolean(*func)(MapObject)
        ...
        for (
                mobj = game.playerSetup.blocklinks[y*ps.bmapwidth+x]; 
                mobj!=null ; mobj = mobj.bnext  ) {
            if (!func.doFunc(mobj) ) {
                return false;
            }
        }
