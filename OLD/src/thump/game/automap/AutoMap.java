...
import static thump.global.Defines.KEY_DOWNARROW;
import static thump.global.Defines.KEY_LEFTARROW;
import static thump.global.Defines.KEY_RIGHTARROW;
import static thump.global.Defines.KEY_TAB;
import static thump.global.Defines.KEY_UPARROW;
import static thump.global.Defines.MAXPLAYERS;
import static thump.global.Defines.PowerType.pw_allmap;
...
import static thump.play.Local.MAPBLOCKUNITS;
import static thump.play.Local.PLAYERRADIUS;
import thump.render.Line;
import static thump.render.Line.ML_DONTDRAW;
...
//    } islope_t;
    ...
    int scale_ftom;
     ...
     Patch marknums[] = new Patch[10]; // numbers used for marking by the automap
...
//        g.fillRect(xx, yy, 1, 1);
        ...
        int 	colorrange)
    {
        ...
        for (i=0;i<game.playerSetup.sectors.length;i++) {
            t = game.playerSetup.sectors[i].thinglist;
            while (t!=null) {
                AM_drawLineCharacter
        ...
        for (i=0;i<AM_NUMMARKPOINTS;i++) {
            if (markpoints[i].x != -1) {
                ...
                if (fx >= f_x && fx <= f_w - w && fy >= f_y && fy <= f_h - h) {
                    game.video.drawPatch(fx, fy, FB, marknums[i]);
                }
            }
        }
