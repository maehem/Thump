...
import static thump.global.Defines.TICRATE;
import thump.global.Random;
import thump.render.Patch;
import static thump.sound.sfx.Sounds.MusicEnum.mus_dm2int;
import static thump.sound.sfx.Sounds.MusicEnum.mus_inter;
import static thump.sound.sfx.Sounds.SfxEnum.sfx_barexp;
import static thump.sound.sfx.Sounds.SfxEnum.sfx_pistol;
import static thump.sound.sfx.Sounds.SfxEnum.sfx_pldeth;
import static thump.sound.sfx.Sounds.SfxEnum.sfx_sgcock;
import static thump.sound.sfx.Sounds.SfxEnum.sfx_slop;
...
//    public static final int SP_PAR		=	ST_TIME;
    ...
    static Patch		yah[] = new Patch[2]; 

    // splat
    static Patch		splat[] = new Patch[2];

    // %, : graphics
    static Patch		percent;
    static Patch		colon;

    // 0-9 graphic
    static Patch		num[] = new Patch[10];

    // minus sign
    static Patch		wiminus;

    // "Finished!" graphics
    static Patch		finished;

    // "Entering" graphic
    static Patch		entering; 

    // "secret"
    static Patch		sp_secret;
    ...
    // "killers", "victims"
    static Patch		killers;
    static Patch		victims; 
    ...
    // "red P[1..MAXPLAYERS]"
    static Patch		p[] = new Patch[MAXPLAYERS];

    // "gray P[1..MAXPLAYERS]"
    static Patch		bp[] = new Patch[MAXPLAYERS];
    ...
    void WI_drawLF() {
        ...
        // draw <LevelName> 
        game.video.drawPatch((SCREENWIDTH - lnames[wbs.last].width)/2,
                    y, FB, lnames[wbs.last]);

        // draw "Finished!"
        y += (5*lnames[wbs.last].height)/4;

        game.video.drawPatch((SCREENWIDTH - finished.width)/2,
                    y, FB, finished);
    }
    ...
    void WI_drawEL() {
        ...
        // draw "Entering"
        game.video.drawPatch((SCREENWIDTH - entering.width)/2,
                    y, FB, entering);

        // draw level
        y += (5*lnames[wbs.next].height)/4;

        game.video.drawPatch((SCREENWIDTH - lnames[wbs.next].width)/2,
                    y, FB, lnames[wbs.next]);

    }

    void WI_drawOnLnode(int n, Patch c[]) {
        ...
        if (fits && i < 2) {
            game.video.drawPatch(lnodes[wbs.epsd][n].x, lnodes[wbs.epsd][n].y,
                    FB, c[i]);
        } else {
            // DEBUG
            Defines.logger.log(Level.WARNING, "Could not place patch on level {0}\n", n + 1);
        }
    }
    ...
    void WI_drawAnimatedBack() {
        ...
        for (int i=0 ; i<NUMANIMS[wbs.epsd] ; i++) {
            ...
            if (a.ctr >= 0) {
                game.video.drawPatch(a.loc.x, a.loc.y, FB, a.p[a.ctr]);
            }
        }
    ...
    int WI_drawNum(int _x, int _y, int _n, int _digits) {
        ...
        while (digits > 0) {
            digits--;
            x -= fontwidth;
            game.video.drawPatch(x, y, FB, num[n % 10]);
            n /= 10;
        }
        ...
        if (neg) {
            x -= 8;
            game.video.drawPatch(x, y, FB, wiminus);
        }
    ...
    void WI_drawPercent(int x, int y, int p) {
        ...
        game.video.drawPatch(x, y, FB, percent);
        WI_drawNum(x, y, p, -1);
    }
    ...
    void WI_drawTime(int _x, int _y, int t) {
        ...
        if (t <= 61 * 59) {
            ...
            do {
                ...
                // draw
                if (div == 60 || (t / div)>0 ) {
                    game.video.drawPatch(x, y, FB, colon);
                }

            } while (t / div>0);
        } else {
            // "sucks"
            game.video.drawPatch(x - sucks.width, y, FB, sucks);
        }
    }
    ...
    void WI_drawDeathmatchStats() {
        ...
        game.video.drawPatch(DM_TOTALSX - (total.width / 2),
                DM_MATRIXY - WI_SPACINGY + 10,
                FB,
                total);

        game.video.drawPatch(DM_KILLERSX, DM_KILLERSY, FB, killers);
        game.video.drawPatch(DM_VICTIMSX, DM_VICTIMSY, FB, victims);
        ...
        for (i = 0; i < MAXPLAYERS; i++) {
            if (game.playeringame[i]) {
                game.video.drawPatch(x - p[i].width / 2,
                        DM_MATRIXY - WI_SPACINGY,
                        FB,
                        p[i]);

                game.video.drawPatch(DM_MATRIXX - p[i].width / 2,
                        y,
                        FB,
                        p[i]);
    ...
    void WI_drawNetgameStats() {
        ...
        // draw stat titles (top line)
        game.video.drawPatch(NG_STATSX+NG_SPACINGX-kills.width,
                    NG_STATSY, FB, kills);

        game.video.drawPatch(NG_STATSX+2*NG_SPACINGX-items.width,
                    NG_STATSY, FB, items);

        game.video.drawPatch(NG_STATSX+3*NG_SPACINGX-secret.width,
                    NG_STATSY, FB, secret);

        if (dofrags>0) {
            game.video.drawPatch(NG_STATSX+4*NG_SPACINGX-frags.width,
                    NG_STATSY, FB, frags);
        }
        ...
        for (i=0 ; i<MAXPLAYERS ; i++)
        {
            ...
            x = NG_STATSX;
            game.video.drawPatch(x-p[i].width, y, FB, p[i]);

            if (i == me) {
                game.video.drawPatch(x-p[i].width, y, FB, star);
            }
    ...
    void WI_drawStats() {
        ...
        game.video.drawPatch(SP_STATSX, SP_STATSY, FB, kills);
        WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY, cnt_kills[0]);

        game.video.drawPatch(SP_STATSX, SP_STATSY+lh, FB, items);
        WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+lh, cnt_items[0]);

        game.video.drawPatch(SP_STATSX, SP_STATSY+2*lh, FB, sp_secret);
        WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);

        game.video.drawPatch(SP_TIMEX, SP_TIMEY, FB, time);
        WI_drawTime(SCREENWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time);

        if (wbs.epsd < 3)
        {
            game.video.drawPatch(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, FB, par);
