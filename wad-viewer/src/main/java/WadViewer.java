...
public class WadViewer extends javax.swing.JFrame implements TreeSelectionListener {
    ...
    private final ArrayList<BufferedImage> imageCache = new ArrayList<>();
    ...
    public WadViewer() {
        ...
        imageScrollPane.setPreferredSize(new Dimension(740, 640));
        imagePanel.add(imageScrollPane);
        
        detailsPane = new JTextPane();
        detailsScrollPane = new JScrollPane(detailsPane);
        //detailsScrollPane.setSize(new Dimension(-1, 200));
        detailsScrollPane.setPreferredSize(new Dimension(640, 300));
        imagePanel.add(detailsScrollPane,BorderLayout.SOUTH);
    ...
    public void valueChanged(TreeSelectionEvent tse) {
        ...
        imagePreviewPanel.setPreviewImage(null);
        ...
        if (node instanceof LumpNode) {
            ...
            if (lump instanceof PictureLump) {
                ...
                //imagePreviewPanel.setPreviewImage(DrawUtils.getRawPatchImage(pl.pic).getScaledInstance(640, -1, Image.SCALE_FAST));
                Image img = DrawUtils.getColorImage(
                        pl.pic, wad.paletteList, 0);
                Image scaledInstance = img.getScaledInstance(img.getWidth(this)*2, -1, Image.SCALE_FAST);
                imagePreviewPanel.setPreviewImage(scaledInstance);
                //logger.log(Level.WARNING, "Selected pic.patchnum: {0}", pl.pic.getPatchNum());
                //imagePreviewPanel.setImage(getColorImage(
                //        wad.patchesLump.getPatch(pl.pic.getPatchNum()),
                //        0/*wad.getPlayPalLump().paletteList.get(0)*/).getScaledInstance(640, -1, Image.SCALE_DEFAULT));
            } else if (lump instanceof PlaypalLump) {
                PlaypalLump pl = (PlaypalLump) lump;
                imagePreviewPanel.setPreviewImage(DrawUtils.getPreviewImage(pl));
            } else if (lump instanceof EndDoomLump) {
                EndDoomLump l = (EndDoomLump) lump;
                //imagePreviewPanel.setImage(l.getAsImage(720,400));
                imagePreviewPanel.setPreviewImage(DrawUtils.getAsImage(l, 720, 400));
                ...
                imagePreviewPanel.setPreviewImage(DrawUtils.getMapImage(l));
            } else if (lump instanceof ThingsLump) {
                logger.log(Level.CONFIG, "{0}\n", lump.toString());
            } else {
                logger.log(Level.CONFIG, "{0}\n{1}\n", new Object[]{lump.name, lump.toString()});
            }
            detailsPane.setText(lump.toString());
        } else if (node instanceof PictureNode) {
            PatchData p = ((PictureNode) node).getPicture();
            imagePreviewPanel.setPreviewImage(getColorImage(p, 0 /*wad.getPlayPalLump().paletteList.get(0)*/));
            detailsPane.setText(p.toString());
        } else if (node instanceof FlatNode) {
            Flat f = ((FlatNode) node).getFlat();
            imagePreviewPanel.setPreviewImage(DrawUtils.getColorImage(wad, f));
            detailsPane.setText(f.toString());
            ...
            Image img = DrawUtils.getTextureImage(wad, n.getTexture());
            imagePreviewPanel.setPreviewImage(img.getScaledInstance(img.getWidth(this)*2, -1, Image.SCALE_FAST));
            detailsPane.setText(n.toString());
            ...
            try {
                ...
                sequencer.setSequence(createMidiSequence(lump.name, lump.mData));
                // Start playing
                sequencer.start();
            ...
            //Sound.playSound(((SoundNode) node).getLump());
            SoundEffectLump sl = ((SoundNode) node).getLump();
            detailsPane.setText(sl.toString());
            try {
        ...
        //imageScrollPane.setViewportView(imagePreviewPanel);
    ...
    private DefaultMutableTreeNode getSpritesNode(Lump l) {
        ...
        while (sprites.hasNext()) {
            n.add(new PictureNode(sprites.nextIndex(), sprites.next()));
        }
    ...
    private void createTexturesNodes(DefaultMutableTreeNode n, Lump l) {
        ...
        ((TextureLump) l).textures.stream().forEach((mt) -> {
...
//        }
        ...
        while (patches.hasNext()) {
            n.add(new PictureNode(patches.nextIndex(), patches.next()));
        }
    ...
    private class ImagePanel extends JPanel {
        ...
        Image blankImage = new BufferedImage(4, 4, BufferedImage.TYPE_INT_ARGB);

        public ImagePanel() {
//            try {
//                previewImage = ImageIO.read(new File(System.getProperty("user.home") + File.separator + "test.jpg"));
...
//                g.drawImage(
//                        //image.getScaledInstance(previewImage.getWidth(this), previewImage.getHeight(this), Image.SCALE_DEFAULT),
//                        previewImage.getScaledInstance(previewImage.getWidth(ImagePanel.this), previewImage.getHeight(ImagePanel.this), Image.SCALE_DEFAULT),
//                        0, 0, null);
        ...
        public void setPreviewImage(Image image) {
            if ( image == null ) {
                this.previewImage = blankImage;
            } else {
                this.previewImage = image;
            }
            
            
            ImagePanel.this.setPreferredSize(new Dimension(previewImage.getWidth(this)+16, previewImage.getHeight(this)+16));
            //ImagePanel.this.setPreferredSize(imagePanel.getSize());
            //this.pack();
            ImagePanel.this.revalidate();
            
            imageScrollPane.validate();
            //detailsScrollPane.validate();
            //imagePanel.setPreferredSize(new Dimension(this.previewImage.getWidth(this), this.previewImage.getHeight(this)));
            //imagePanel.validate();
    ...
    private class PictureNode extends DefaultMutableTreeNode {
        ...
        public PictureNode(int patchNum, PatchData p) {
        ...
        public String toString() {
            //return picture.name;
            return patchNum + ": " + picture.toString();
        }
    ...
    private void initComponents() {
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addComponent(imagePanel, javax.swing.GroupLayout.DEFAULT_SIZE, 540, Short.MAX_VALUE)
                .addContainerGap())
        );
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(imagePanel, javax.swing.GroupLayout.PREFERRED_SIZE, 696, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    public Image getColorImage(PatchData pd, int paletteNum) {
        BufferedImage img;
//        try {
...
//        }
        ...
        if (img != null) {
            ...
            img = new BufferedImage(pd.width, pd.height, BufferedImage.TYPE_INT_ARGB);
            int x = 0;
            int[] palette = wad.paletteList.get(paletteNum);
        ...
        return img.getScaledInstance(img.getWidth()*2, -1, Image.SCALE_FAST);
    }
