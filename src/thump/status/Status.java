...
import thump.global.Cheat;
import thump.global.Defines;
import static thump.global.Defines.AmmoType.am_noammo;
import static thump.global.Defines.GameMode.*;
import static thump.global.Defines.KEY_ENTER;
import static thump.global.Defines.MAXPLAYERS;
import thump.global.Defines.PowerType;
import static thump.global.Defines.PowerType.*;
import static thump.global.Defines.SCREENHEIGHT;
import static thump.global.Defines.SCREENWIDTH;
import static thump.global.Defines.SCREEN_MUL;
...
import thump.play.Interaction;
import thump.render.Patch;
import thump.sound.sfx.Sounds;
import static thump.sound.sfx.Sounds.MusicEnum.mus_e1m1;
import static thump.sound.sfx.Sounds.MusicEnum.mus_runnin;
import static thump.status.Status.ChatStateNum.StartChatState;
import static thump.status.Status.StateNum.AutomapState;
import static thump.status.Status.StateNum.FirstPersonState;
...
// States for status bar code.
    ...
    Patch   sbar;    // main bar left
    Patch   tallnum[]       = new Patch[10];    // 0-9, tall numbers
    Patch   tallpercent;    // tall % sign
    Patch   shortnum[]      = new Patch[10];    // 0-9, short, yellow (,different!) numbers
    Patch   keys[]          = new Patch[Defines.Card.values().length];     // 3 key-cards, 3 skulls
    Patch   faces[]         = new Patch[ST_NUMFACES];    // face status patches
    Patch   faceback;    // face background
    Patch   armsbg;     // main bar right
    Patch   arms[][] = new Patch[6][2];     // weapon ownership patches
    ...
    void ST_refreshBackground() {
        ...
        if (st_statusbaron) {
            ...
            Game.getInstance().video.drawPatch(ST_X, 0, BG, sbar);

            if (Game.getInstance().netgame) {
                Game.getInstance().video.drawPatch(ST_FX, 0, BG, faceback);
            }

            Game.getInstance().video.copyRect(ST_X, 0, BG, ST_WIDTH, ST_HEIGHT, ST_X, ST_Y, FG);
        }
    }
...
//                    plyr.message  = buf;
    ...
    void ST_doPaletteStuff() {
        ...
        if (palette != st_palette) {
            st_palette = palette;
            Defines.logger.log(Level.CONFIG, "Do_Palette_Stuff: Set Palette {0}\n", palette);
