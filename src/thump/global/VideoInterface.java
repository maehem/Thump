...
public class VideoInterface implements KeyListener, ComponentListener {
    ...
    public void I_UpdateNoBlit () {
        ...
        for (int sn = 0; sn<5; sn++) {
            //viewCanvas.repaint();
            int[] area = Game.getInstance().video.screens[sn].area;
            BufferedImage img = Game.getInstance().video.screenImage[sn];
...
//    }
    ...
    public void I_ReadScreen (Screen scr){
        ...
        System.arraycopy(Game.getInstance().video.screens[0].area, 0, scr.area, 0, SCREENWIDTH*SCREENHEIGHT); //scr.area = Arrays.copyOf(Game.getInstance().video.screens[0].area, SCREENWIDTH*SCREENHEIGHT);
    }
