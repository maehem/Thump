...
import static thump.sound.sfx.Sounds.MusicEnum.mus_bunny;
import static thump.sound.sfx.Sounds.MusicEnum.mus_evil;
import static thump.sound.sfx.Sounds.MusicEnum.mus_read_m;
import static thump.sound.sfx.Sounds.MusicEnum.mus_victor;
...
//        }
        for ( int y=0; y<SCREENHEIGHT; y+=64 ) {
            for( int x=0; x<SCREENWIDTH; x+=64 ) {
                game.video.drawPatch(x, y, dest, src);
            }
        }
...
//                            break;
//                        }
                        game.video.drawPatch(cx, cy, 0, game.headUp.hu_font[c]);
                        cx += w;
                    }
                }
...
//    } CastInfo;
    ...
    void F_CastPrint(String text) {
        ...
        while (chix<text.length()) {
            ...
            w = game.headUp.hu_font[c].width;
            game.video.drawPatch(cx, 180, 0, game.headUp.hu_font[c]);
            cx += w;
        }
    ...
    void F_CastDrawer ()
    {
        ...
        game.video.drawPatch (0,0,0, game.wad.getPatchByName("BOSSBACK"));
        ...
        if (flip) {
            game.video.drawPatchFlipped (160,170,0,patch);
        } else {
            game.video.drawPatch (160,170,0,patch);
        }
    }
      ...
      int		col )
    {
        ...
        column.draw(game.video.screens[0], x,0);
//        byte*	source;
//        byte*	dest;
//        byte*	desttop;
//        int		count;
...
//        }
    ...
    void F_BunnyScroll ()
    {
        ...
        if (finalecount < 1180)
        {
            game.video.drawPatch ((SCREENWIDTH-13*8)/2,
        ...
        name = "END" + stage;
        game.video.drawPatch ((SCREENWIDTH-13*8)/2, (SCREENHEIGHT-8*8)/2,0, game.wad.getPatchByName(name));
    }
