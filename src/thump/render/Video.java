...
//	} 
    ...
    public void drawPatchDirect(int x, int y, int scrn, String lumpName) {
        if (lumpName == null || "".equals(lumpName)) {
            return;
        }

        Lump lump = Game.getInstance().wad.findByName(lumpName);
        if (lump instanceof PictureLump) {
            drawPatchDirect(x, y, scrn, ((PictureLump) lump).pic);
        }
    }
    ...
    //
    public void drawPatchDirect(int x,
            int y,
            int scrn,
            Patch patch) {
