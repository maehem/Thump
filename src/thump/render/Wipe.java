...
//        int		x;
    ...
    public int wipe_StartScreen(
            ...
            int width,
            int height) {
        wipe_scr_start = Game.getInstance().video.screens[2];
    ...
    public int wipe_EndScreen(
            ...
            int width,
            int height) {
        wipe_scr_end = Game.getInstance().video.screens[3];
        VideoInterface.getInstance().I_ReadScreen(wipe_scr_end);
        //copyScreen(wipe_scr_start, Game.getInstance().video.screens[0]);
        Game.getInstance().video.drawBlock(x, y, 0, width, height, wipe_scr_start); // restore start scr.
        return 0;
    }
...
//        };
        ...
        if (!go) {
            ...
            wipe_scr = Game.getInstance().video.screens[0];
            //(wipes[wipeno*3])(width, height, ticks);
            doWipe(wipeno.ordinal() * 3, width, height, ticks);
        }

        // do a piece of wipe-in
        Game.getInstance().video.markRect(0, 0, width, height);
