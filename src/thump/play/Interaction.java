...
import thump.global.Defines.Card;
import static thump.global.Defines.Card.*;
import static thump.global.Defines.PowerDuration.*;
import thump.global.Defines.PowerType;
import static thump.global.Defines.PowerType.*;
import static thump.global.Defines.Skill.sk_baby;
...
import static thump.play.Local.BASETHRESHOLD;
...
//                S_StartSound (NULL, sfx_wpnup);
            ...
            MapObject	target )
    {
        ...
        {
            Game.getInstance().movingObject.P_SetMobjState (target, target.info.xdeathstate);
        }
        else {
            Game.getInstance().movingObject.P_SetMobjState (target, target.info.deathstate);
        }
...
//            }
             ...
             && (target.flags&MF_SKULLFLY.getValue())==0 )
        {
            ...
            Game.getInstance().movingObject.P_SetMobjState (target, target.info.painstate);
        }
             ...
             && source.type != MT_VILE)
        {
            ...
            if (target.state == states[target.info.spawnstate.ordinal()]
                ...
                Game.getInstance().movingObject.P_SetMobjState (target, target.info.seestate);
            }
        }
